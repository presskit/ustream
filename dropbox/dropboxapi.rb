...
module Dropbox
  ...
  class Oauth
    ...
    def initialize(info)
      @app_key    = info.get_app_key    #"qrvbtkkv4r6fjgd"
      @app_secret = info.get_app_secret #"r66rglltfvt1we2"
...
#account_info = Dropbox::AccountInfo.new(oauth).open.get_json
metadata = Dropbox::Metadata.new(oauth).open("sandbox","/ustream.list.txt").get_json
puts metadata["path"]
